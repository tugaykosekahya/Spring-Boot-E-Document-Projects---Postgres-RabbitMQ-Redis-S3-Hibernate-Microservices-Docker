package tr.gov.gib.evdbelge.evdbelgesorgulama.dao.impl;
...
public class BelgeDaoJpaImpl implements BelgeDaoJpa {
    ...
    public long nativeInsert(String queryString, boolean idDonulecek) throws GibRemoteException, GibBusinessException {
        /*Query query = entityManager.createNativeQuery(queryString, Tuple.class);
        List<Tuple> resultList = query.getResultList();*/
        ...
        nativeQuery.setResultTransformer(AliasToEntityMapResultTransformer.INSTANCE);
        if(idDonulecek) {
            Map map = (HashMap) nativeQuery.getSingleResult();
            if(map != null && map.get("id") != null) {
                long id =  Long.parseLong(map.get("id").toString());
                System.out.println(id);
                return id;
            }
            throw new GibBusinessException("id bilgisi dönmedi", "");
        }
        else {
            int checkZero = nativeQuery.executeUpdate();
            if(checkZero == 0)
    ...
    @Override
    public void belgeUpdate(long belgeId, int durum, String nextnodeuserid) throws GibException {
        ...
        query.set(root.get("nextnodeuserid"), nextnodeuserid);
